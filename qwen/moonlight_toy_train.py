...
def get_model_and_dataloader(model_name, dataset_name, hidden_size):
    name2path = {
        "openwebtext-100k": "./data/openwebtext-100k",
    }
...
def get_optimizer(optimizer_name, model, lr=1e-3, wd=0.1):
    ...
    elif optimizer_name == "mgup_adamw":
        return MGUP_AdamW(
            model.parameters(), lr=lr, weight_decay=wd, betas=(0.9, 0.95)
        )
...
if __name__ == "__main__":
    ...
    # Initialize wandb
    run_name = f"{args.model}_{args.optimizer}_lr{args.lr}_wd{args.wd}"
    wandb.init(
        project="moonlight-training",
        name=run_name,
        config={
            "model": args.model,
